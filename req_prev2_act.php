...
switch($act)
{
    case "add":
		...
		{
			$mainlib->closedb();
			$mainlib->redirect("msg.php?msg=".urlencode("Data sudah ada")."&url=");
		}
        ...
        $mainlib->dbquery("INSERT INTO jadwal_prev2(gedung,lantai,lokasi,barang,jenis,kapasitas,leader,periode,teknisi,teknisi2,teknisi3,tglakhir,status) VALUES ('".$vgedung."','".$vlantai."','".$vlokasi."','".$vbarang."','".$vjenis."','".$vkapasitas."','".$vleader."','".$vperiode."','".$vteknisi."','".$vteknisi2."','".$vteknisi3."','".$vtglakhir."','Progress')", $mainlib->ocn);
        $mainlib->dbquery("INSERT INTO jadwal_prev2_ac(gedung,lantai,lokasi,barang,jenis,kapasitas,leader,gol,periode,teknisi,teknisi2,teknisi3,tglkerja,status) VALUES ('".$vgedung."','".$vlantai."','".$vlokasi."','".$vbarang."','".$vjenis."','".$vkapasitas."','".$vleader."','".$vgol."','".$vperiode."','".$vteknisi."','".$vteknisi2."','".$vteknisi3."','".$vtglakhir."','Progress')", $mainlib->ocn);
       ...
       if ($vtglakhir > $vtgl_hrini)
		...
		if (isset($_REQUEST["hidIDs"]))
		{
			if (trim($_REQUEST["hidIDs"])!="")
			{
		        ...
		        $sql = "DELETE FROM jadwal_prev2 WHERE ".$sql;
...
$mainlib->closedb();
$mainlib->redirect("req_prev2_list.php?pgret=1");
?>		
