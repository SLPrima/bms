<?php
	...
	if(isset($_REQUEST["pgret"]))
	{
		...
		$filtercol2 = $_SESSION[$listformbase_sess_prefix."listfiltercol2".$list_sess_id];
		$filtertext2 = $_SESSION[$listformbase_sess_prefix."listfiltertext2".$list_sess_id];
	}else{
		$pageno = $_REQUEST["hidPage"];
		$ordercol = $_REQUEST["hidOrder"];
		$filtercol = $_REQUEST["cboFilter"];
		$filtertext = $_REQUEST["txtFilter"];
		$filtercol2 = $_REQUEST["cboFilter2"];
...
?>
	...
	function releaseFilter()
	{
		...
		document.frm.cboFilter2.selectedIndex = 0;
		document.frm.txtFilter2.value = '';
		document.frm.submit();
	}
		...
		<td colspan="<?=$colcount?>">
			...
			<tr>
				...
				<td class="tablecoldetail">
					...
					<select name="cboFilter2" class="inputtext">
					<?php
					for($i=0;$i<count($arfiltercolname);$i++)
					{
						echo '<option value="'.$arfiltercolname[$i].'"';
						if(strtolower($arfiltercolname[$i])==strtolower($filtercol2))
						{
							echo ' selected';
						}
						echo '>'.$arfiltercolcaption[$i].'</option>';
...
<?php
	 ...
	 if($filtertext2=="")
	  {
 	    ...
 	    if (substr($filtercol,0,3) == "tgl")
	     {
	      ...
	      $filter33 = substr($filtertext,6,4);     
	      $filtertext = $filter33."-".$filter22."-".$filter11;  
    	  $sql_filter = $filtercol." LIKE '%".$filtertext."%' "." and ". $filtercol2. " LIKE '%".$filtertext2."%' ";
	     } else {
    	  $sql_filter = $filtercol." LIKE '%".$filtertext."%' "." and ". $filtercol2. " LIKE '%".$filtertext2."%' ";
	     } 
	...
	$_SESSION[$listformbase_sess_prefix."listfiltercol2".$list_sess_id] = $filtercol2;
	$_SESSION[$listformbase_sess_prefix."listfiltertext2".$list_sess_id] = $filtertext2;
	...
	if($sql_order!="")
	{
	    $sql_order = "gedung,lantai,lokasi";
	...
	while($row = $mainlib->dbfetcharray($rs))
	{
		...
		if($allow_edit)
		{
		    if(strlen($idcol2)>1)
		    {
  			   echo '<td><a href="'.$listeditpage.'?id='.urlencode($row[$idcol]).';'.urlencode($row[$idcol2]).';'.urlencode($row[$idcol3]).';'.urlencode($row[$idcol4]).';'.urlencode($row[$idcol5]).'" title="edit"><img src="images/edit.gif" border=0 align="absmiddle"></a></td>';
            } else {
